...
**На полном датасете ClickBench (76 ГБ, hits):**

| Метрика | Значение |
|---|---|
| Размер после сжатия | 15 ГБ (5x) |
| CSV → .skewdb | 1875s (~40 МБ/с) |
| Все 43 запроса | 926s |

**На выборке (803 МБ, 1М строк, hits_sample):**

| Метрика | Значение |
|---|---|
| Размер после сжатия | 119 МБ (6.5x) |
| CSV → .skewdb | ~9.4s |
| .skewdb → CSV | ~6.4s |
| Все 43 запроса | ~7.2s |
